...
public:
    ...
    bool addPacket(int source, int destination, int timestamp) {
        ...
        // cehcking if already present or not 
        if(packetStore.find(key) != packetStore.end()) return false;

        if(que.size() >= MAX_SIZE){
            forwardPacket(); // delete the front element of the queue
        }
    ...
    vector<int> forwardPacket() {
        ...
        que.pop();
        vector<int> packet = packetStore[key]; // returns a vetcor -->{S,D,T}
        packetStore.erase(key); // removing the element from packetStore map
        // I want the destination packet[1]
        int D = packet[1];
        destTimeMap[D].erase(destTimeMap[D].begin()); // {T1,T2,T3}--> removes T1
        return packet;
    }
